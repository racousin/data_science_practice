...
## What is not initialized at random

Zero, or a constant, is only wrong where it creates symmetry. A layer whose
*inputs* already differ from unit to unit can start at zero, and several
important ones do:

| Parameter | Initial value | Why |
|---|---|---|
| biases | 0 | the weights already break the symmetry |
| last layer of a residual branch, or its norm's $\gamma$ | 0 | the block starts as the identity — [Skip Connections](https://ml-arena.com/courses/ms2a-machine-learning-practice/s4-advanced-neural-networks/skip-connections) |
| LoRA's $B$ matrix | 0 | the adapter is a no-op at step 0 — [Session 8](https://ml-arena.com/courses/ms2a-machine-learning-practice/s8-nlp-2/instruction-tuning-and-peft) |
| output bias of an imbalanced classifier | $\log\frac{\pi}{1-\pi}$ | the first prediction is the base rate $\pi$ |
| LSTM forget-gate bias | 1 | the cell remembers by default |
| policy head in PPO | orthogonal, gain 0.01 | the first policy is near uniform — [Session 10](https://ml-arena.com/courses/ms2a-machine-learning-practice/s10-reinforcement-learning-2/making-deep-rl-work) |

The output bias deserves its line. With 1% positives and a zero bias, the first
prediction is 0.5 for every sample, and the first few hundred steps are spent
learning nothing but the base rate. Starting from the prior removes that phase:

```python
p = y_train.float().mean()                       # 0.01
nn.init.constant_(model.head.bias, math.log(p / (1 - p)))   # -4.6
```

The LSTM one is two lines, because PyTorch packs the four gates — input,
forget, cell, output — into one bias vector:

```python
h = lstm.hidden_size
for name, b in lstm.named_parameters():
    if "bias" in name:
        b.data[h:2 * h].fill_(1.0)           # the forget gate
```

And the best initialization of all is not random either: someone else's trained
weights. That is
[Transfer Learning](https://ml-arena.com/courses/ms2a-machine-learning-practice/s5-computer-vision-1/transfer-learning)
in Session 5, and it beats every rule on this page whenever a pretrained model
for your kind of input exists.
...
1. Run this. You should get exactly the output shown.
   ...
   # -> tensor([[0.4305, 0.3828, 0.4442, 0.4281],
   # ->         [0.4305, 0.3828, 0.4442, 0.4281],
   # ->         [0.4305, 0.3828, 0.4442, 0.4281]])
   ```

   The weights moved, so the network learned something. Why are the three rows
   still identical, and how many hidden units does this network really have?

2. Run this too, and explain the last number.

   ```python
   import math, torch, torch.nn as nn
   torch.manual_seed(0)
   lin = nn.Linear(784, 512)
   print(f"{lin.weight.std():.4f}")          # -> 0.0206
   nn.init.kaiming_normal_(lin.weight, nonlinearity="relu")
   print(f"{lin.weight.std():.4f}")          # -> 0.0505
   print(f"{math.sqrt(2 / 784) * math.sqrt(3 * 784):.3f}")   # -> 2.449
   ```

3. A ten-class model prints a first loss of 38.2. Name two causes, and the one
   line you would print next to tell them apart.

4. Your binary classifier sees 1% positives. What bias do you give the output
   unit, what is the first loss then, and what was it with a zero bias?

5. Zero-initializing the hidden layers of an MLP is a bug; zero-initializing the
   last layer of every residual branch is a recommendation. What is different
   about the second case?

6. A classmate re-initializes a transformer with
   `for p in model.parameters(): nn.init.normal_(p, std=0.02)`. It trains,
   badly. What happened to the LayerNorm layers, and what would have happened
   with `kaiming_normal_` in the same loop?
