...
- **No gap.** If training and validation accuracy agree, dropout only slows
  training down.
- **Enough data.** Most recent pre-training runs of large language models use a
  dropout of 0: the data is barely repeated, so there is little to memorize.
- **A clock.** Under a fixed time budget, dropout converts steps you cannot spare
  into regularization you do not need. In
  [1 Minute Permuted MNIST](https://ml-arena.com/viewchallenge/8), with 60,000
  images and 60 seconds, Lab 4's Part 4 measured it: the gap closed, validation
  accuracy barely moved.
- **Reinforcement learning.** The targets move already; extra noise in the value
  estimate slows learning more than it regularizes.
- **Directly before a normalization layer** — the shift above: free to avoid,
  so avoid it.
...
1. Run this. You should get exactly the output shown.
   ...
   y = drop(x)
   print(f"{(y == 0).float().mean():.3f} {y.max():.4f} {y.mean():.3f}")
   # -> 0.251 1.3333 0.999
   drop.eval()
   print(torch.equal(drop(x), x))          # -> True
   torch.manual_seed(0)
   print(F.dropout(torch.ones(6), 0.5))
   # -> tensor([0., 0., 2., 0., 0., 0.])
   ```

   Where do 0.251, 1.3333 and 0.999 come from? The last line was called
   outside any model and outside `train()`: why did it drop anything?

2. A network with `Dropout(0.5)` has a training loss that stalls at 0.2 while the
   same network without dropout reaches 0.001. Is something wrong?

3. You evaluate the same model twice on the same batch, in `eval()` mode, and get
   different predictions. Name the two places to look first.

4. Why does `Dropout2d` rather than `Dropout` belong after a `Conv2d`?

5. For Monte Carlo dropout you call `model.train()` before the $T$ passes. The
   network has BatchNorm layers. What goes wrong, and what should you call
   instead?

6. Your agent for challenge 8 uses `Dropout(0.5)` and trains for 40 seconds.
   What would you expect from removing it, and why?
