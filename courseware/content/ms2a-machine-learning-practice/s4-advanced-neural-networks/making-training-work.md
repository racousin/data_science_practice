...
A loop that runs is not a loop that learns. Between the two sit seven choices —
activation, initialization, normalization, dropout, depth, loss, and what you do
when the gradient misbehaves. Each one is cheap to change and each one is the
difference between a network that reaches 0.97 and the identical network that
sits at 0.10 forever.
...
<!-- notes: 40 minutes, the core of the session. Do not re-teach layers (lesson 78)
or optimizers and schedules (lesson 80). The two live demos worth the time: print
the dead-unit fraction while raising the learning rate, and run the 24-block plain
vs residual pair — the plain one sits at 2.303 on screen and nobody forgets it.
The clipping section is where RNN people get religion. -->
...
|---|---|---|
| loss pinned at $\ln 10 = 2.303$ | nothing reaches the early layers | initialization, residuals |
...
| train and eval differ by points | BatchNorm statistics, dropout | normalization, dropout |
...
this lesson is what to change once you have read it.
...
$$
\sigma^2_{\text{Xavier}} = \frac{2}{fan_{in} + fan_{out}} \qquad
\sigma^2_{\text{He}} = \frac{2}{fan_{in}}
$$

Xavier keeps the variance constant through a symmetric activation; He doubles it
to compensate for ReLU throwing half of the distribution away. That factor of two
is the entire difference, and it compounds thirty times.
...
**PyTorch's defaults are not He.** `nn.Linear` initialises from
$\mathcal{U}(-1/\sqrt{fan_{in}},\, 1/\sqrt{fan_{in}})$, a standard deviation of
$1/\sqrt{3\,fan_{in}}$ — 0.0206 for `Linear(784, 512)`, against 0.0505 for He.
They are deliberately conservative and they are fine for the five- or ten-layer
networks in this course; the figure is what happens when you stack thirty of them
without saying anything.

Where initialization really bites is a parameter you create yourself.
`torch.randn(512, 784)` has $\sigma = 1$, about 20× wider than He for that
fan-in, and it produces `nan` in a handful of steps:

```python
self.w = nn.Parameter(torch.randn(512, 784) * (2 / 784) ** 0.5)   # He, by hand
```

Two things that are *not* initialised randomly: biases go to zero, and the
$\gamma$ of the last norm layer inside a residual block is often set to zero so
the block starts as the identity.
...
free point of accuracy — the 24-block stack further down does not train at all
without it. Four ways BatchNorm bites:
...
- **Dropout immediately before it.** Dropout's output variance differs between
  the two modes, so the norm accumulates statistics that are wrong at inference.
  Put dropout after the norm, or leave a Linear between them.
...
nn.Dropout1d(p=0.1)    # drops whole channels — the right one after a Conv
```

Place it where the parameters are: a 4096-unit classifier head deserves 0.5, a
convolutional stack rarely wants more than 0.1, and a narrow bottleneck wants
none — there, dropout destroys information instead of regularising it. Two
consequences worth remembering: dropout always slows convergence (you are
training a different sub-network every step), and it puts a floor under the
training loss, which is why the overfit-one-batch test in
[Monitoring and Debugging](https://ml-arena.com/courses/ms2a-machine-learning-practice/s4-advanced-neural-networks/monitoring-and-debugging)
is run with dropout off.

Reach for it *after* you have seen a validation curve turn up, not before. With
BatchNorm already in the block and a real dataset, the first thing to try is
usually less dropout, not more.
...
Differentiate: $\partial y/\partial x = \partial F/\partial x + I$. The identity
term means the gradient reaching layer 1 no longer has to survive a product of
forty Jacobians — there is a path along which it survives untouched.
...
That is the ResNet result in miniature, and the point of the 2015 paper: a plain
56-layer network had *higher training* error than a plain 20-layer one. The
failure is optimization, not generalisation, and the identity path fixes it.
...
class Block(nn.Module):
    ...
    def forward(self, x):
        h = self.fc2(F.relu(self.fc1(self.norm(x))))   # pre-norm
        return x + self.drop(h)
```

Three rules come with it:

- **The block must return the shape it received**, or the addition is illegal.
  When the width or the stride changes, project the shortcut —
  `nn.Conv2d(c_in, c_out, kernel_size=1, stride=s)` — rather than dropping it.
- **Normalise before the block, not after.** Pre-norm (above) trains without
  warmup and tolerates a much wider range of learning rates; post-norm — the
  original 2017 transformer — needs a warmup schedule to survive its first
  hundred steps.
- **Add the skip, then add depth.** Beyond about 15 weight layers, a network
  without residual connections is a network you are debugging, not training.
...
Every choice in that line is one of the sections above, and the combination
trains at almost any depth. Change one thing at a time and keep the run that
justified the change — three variants at once tell you nothing, and
[Monitoring and Debugging](https://ml-arena.com/courses/ms2a-machine-learning-practice/s4-advanced-neural-networks/monitoring-and-debugging)
...
1. A network's loss sits at exactly 2.303 for twenty epochs on a ten-class
   problem. Name three causes from this lesson, and say which figure above shows
   each one.
