...
## What engineering leaves behind

- ratios, differences and rates — the arithmetic a model cannot do for itself
- datetime components on a circle, plus the domain calendar no library ships
- bins for a linear model, never for a tree
- per-group statistics from train rows only, with the target treated as in
  target encoding

Every one of these is a column the model now has to justify, and the next
lesson is about taking back the ones that cannot. One question comes first,
before any of them reaches a model: could this value be computed at prediction
time?
...
3. Why is `clip(lower=1)` in the debt-to-income line not cosmetic?

   **Answer.** A zero income produces `inf`, which is not `NaN`: it passes every
   `isna()` check and the imputer never touches it. The clip turns the division
   into a number before any downstream step can mistake it for a valid value.
