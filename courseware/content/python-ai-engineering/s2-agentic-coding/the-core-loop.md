...
1. Name the four steps of the cycle in order, then name the two where your
   leverage is highest.

   **Answer.** Explore, Plan, Act, Verify. Leverage is highest at **Plan** and
   **Verify** — which are the same two steps students skip.

2. Your repository has no tests. Which step of the loop breaks, and what does
   the agent do instead?

   **Answer.** Verify breaks: with nothing to check against, the agent stops
   when the code *looks* finished. That is why "no tests means no loop", and it
   is the strongest practical argument for Session 1's test suite.

3. Here is a prompt: *"add caching"*. Rewrite it using the four ingredients,
   then check your version against the lesson's.

   **Answer.** The four ingredients are **where** (the file or function by
   name), **what** (the observable behaviour), **how you will know** (the tests
   or the command that must pass), and **what must not change** (signatures,
   dependencies, layout). The lesson's rewrite names `load_dataset()` in
   `src/textstats/data.py`, an mtime-invalidated cache keyed by absolute path,
   three specific tests, and "keep the public signature unchanged".

4. The agent starts editing `tests/` when you asked it to change `src/`. Why
   does the lesson tell you to interrupt and correct rather than `/clear` and
   start again?

   **Answer.** The context is preserved across an interrupt, so a correction is
   cheaper than a restart — restarting throws away the exploration you already
   paid for. `/clear` is for the other cases: it has looped twice on the same
   failure, the conversation has drifted, or you changed your mind about the
   approach.
