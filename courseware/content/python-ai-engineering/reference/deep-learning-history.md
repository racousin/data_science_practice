...
1. AI, ML and DL are nested sets. What single property separates the innermost
   from the one containing it?

   **Answer.** Learned features. Classical machine learning takes the features you
   engineered; deep learning derives its own representations from raw input.

2. Convolutional networks date from 1989 and AlexNet is a convolutional network.
   So why is 2012 the dividing line?

   **Answer.** Not the architecture — the conditions. ImageNet's 14M labelled
   images, GPUs made generally programmable by CUDA, and the algorithmic details
   (ReLU, dropout) together made it work publicly and by about 10 points. The
   field waited on data, compute and detail, not on a theoretical breakthrough.

3. What does this page say the bottleneck usually is, and what does it tell you
   to do instead of training a vision or language model from scratch?

   **Answer.** The bottleneck is usually data, not architecture — it has been for
   most of this timeline. Start from a pretrained model and adapt it; training
   from scratch is almost never the right move at your scale. And old ideas are
   not dead ideas: gradient boosting still wins on tabular data, and a linear
   baseline still tells you what your network has to beat.
