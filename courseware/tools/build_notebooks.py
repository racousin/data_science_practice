...
* `aie-s3-bike-demand.ipynb` — **worked**. The same challenge as Session 2
  (183), with the feature engineering deliberately left out: split first
  (chronological), transform second, compare nine candidates on train *and*
  validation, then `GridSearchCV`, then spend the test set once. -138.88 ->
  -48.06 with no new column.
* `aie-s3-bank-marketing.ipynb` — **guided**. Challenge 184 again, same
  protocol on a classification target, plus the decision threshold — which on
  an 11.7%-positive target is worth more than the model swap. No code.
...
The Session 3 pair reads those Session 2 files and rebinds them on the way in
(`X = read_csv("X_train.csv")`, `X_submission = read_csv("X_test.csv")`), which
is how the notebook that teaches the split gets the name `X_test` back for the
slice it holds out. The rename is called out in the notebook, because it is the
first thing the session changes.
"""
...
# modelling and the evaluation. -138.88 -> -48.06 with no new feature.
#
# Every number quoted in the prose below was measured against the committed
# CSVs in competitions/s2-bike-demand/data/ with the exact code in the cells.
# --------------------------------------------------------------------------- #
def build_bike_modeling() -> dict:
    ...
    return notebook([
        ...
        md("---", "", "## 2. Read it, and name it for what it is"),
        code(
            "import pandas as pd",
            "",
        ...
        md("The rename is deliberate, and it is the first thing this session",
           "changes.",
           "",
        ...
        md("---", "", "## 3. Split first — before you touch a single value",
           "",
        ...
        md("`8,899 / 2,224 / 2,780`. Note the target means: **143 / 184 / 268**.",
        ...
        md("---", "", "## 4. Then transform — the minimum that works",
           "",
        ...
        md("19 columns out of 12 features. And **`hour` is still a plain",
        ...
        md("---", "", "## 5. Compare models — two numbers per candidate",
           "",
        ...
        md("| model | hyperparameters | train MAE | val MAE |",
           "|---|---|---|---|",
        ...
        md("The linear rows have two bars almost the same length; the deep-forest",
           "rows have a stub and a full bar. That gap is memorisation, drawn."),
        md("---", "", "## 6. Search the hyperparameters properly",
           "",
        ...
        md("The winner is `max_depth=16, max_features=1.0, min_samples_leaf=1,",
        ...
        md("---", "", "## 8. Refit on everything, then submit",
           "",
        ...
        md("**−48.06**, against the **−138.88** your Session 2 line scored on the",
        ...
        md("---", "", "## 9. Where to go from here",
           ...
           "  different defaults.",
           "- **Support vector regression.** `SVR(kernel=\"rbf\", C=..., "
           "gamma=...)`",
...
# --------------------------------------------------------------------------- #
# 4. Bank term deposit, again — the guided notebook for Session 3
#
# Challenge 184, the same one Session 2's guided notebook submits to. No code:
# the student has just read the bike notebook and writes this one. The protocol
# is the same with one addition — the decision threshold, which on this target
# is worth more than the model swap (measured: HGB 0.5529 at 0.50, 0.6395 at
# the validation-tuned 0.24).
# --------------------------------------------------------------------------- #
def build_bank_modeling() -> dict:
    name = "aie-s3-bank-marketing.ipynb"
    return notebook([
        md(
        ...
        md("---", "", "## 1. Get the data",
           "",
           f"Connect with your `mlk_user_...` key and download challenge",
           f"**{BANK_ID}** into the working directory. Three files:",
        ...
        code(""),
        code(""),
        md("---", "", "## 6. Search the hyperparameters",
           "",
        ...
        code(""),
        code(""),
        md("---", "", "## 7. The threshold — the knob that is actually worth turning",
           "",
        ...
        code(""),
        md("---", "", "## 9. Refit on everything, then submit",
           "",
...
# --------------------------------------------------------------------------- #
# 8. Session 4 warm-up 2 — CPU vs GPU
#
...
# --------------------------------------------------------------------------- #
# 9. California housing — the worked notebook for Session 4
#
...
def build_california() -> dict:
    ...
    return notebook([
        ...
        md("---", "", "## 10. Predict the submission set, and look at the predictions",
           "",
        ...
        md("**What to expect.** This notebook scores **R² ≈ 0.776** on the",
...
# --------------------------------------------------------------------------- #
# 10. Forest cover type — the guided notebook for Session 4
#
...
def build_forest_cover() -> dict:
    ...
    return notebook([
        ...
        md("---", "", "## 2. Read it, and answer three questions",
           ...
           "",
           "- What are **n** and **p**?",
           "- **How many distinct classes are in `y`, and what are their",
        ...
        md("---", "", "## 3. A quick look",
           ...
           "  `X.describe().loc[[\"min\", \"max\"]].T`. Ten columns are real",
        ...
        md("---", "", "## 9. Predict and submit",
           "",
           "Transform `X_submission` with the **same scaler** — the one fitted in",
           "section 4, not a new one. Predict, argmax, add one.",
           "",
           "Before uploading, assert all four of these. Each corresponds to a real",
           "way this goes wrong:",
           "",
           "- one row per id in `X_submission`, and the ids are unique",
...
def main() -> None:
    ...
    for builder in (build_preflight, build_bike, build_bank,
                    build_bike_modeling, build_bank_modeling,
