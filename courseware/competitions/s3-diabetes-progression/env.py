"""Session 3 — Diabetes progression (file_v1).

Predict a quantitative measure of disease progression one year after baseline,
from ten clinical measurements taken at baseline. 265 training rows.

This is the challenge Session 3 is built around, and it is small and noisy on
purpose. A random forest fits the training set to R2 = 0.92 here and scores
0.49 on these held-out rows; plain linear regression fits to 0.51 and scores
0.52. The forest looks twice as good right up until the moment it is measured
on something it has not seen.
...
`prediction` is a real number on the target's scale (roughly 25 to 350).

Ranking is on **R2**, not RMSE. The leaderboard sorts descending
...
training mean. RMSE and MAE are shown alongside in the target's units.
