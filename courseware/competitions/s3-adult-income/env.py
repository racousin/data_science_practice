"""Session 3 — Adult census income (file_v1).

Lab 3 builds a leak-free pipeline on the UCI Adult dataset and reports an
honest cross-validated F1. This competition closes the loop: the same features,
a held-out test split you never see the labels for, and one number.
...
`prediction` is 1 for income >50K and 0 otherwise. Every test id must appear
exactly once; extra ids are an error, and so is any value other than 0 or 1.

Primary score is F1 on the positive class — the metric Lab 3 argues for, since
the target is ~24% positive and accuracy rewards predicting "no" forever. The
leaderboard also shows accuracy, precision and recall so the tradeoff you made
at the threshold is visible.
                ...
                for line_no, row in enumerate(reader, start=2):
                    ...
                    if value not in VALID:
                        return None, (
                            ...
                            f"expected 0 or 1 (1 means income >50K)."
                        )
