"""Session 2 — DPE energy label (file_v1). You do the preprocessing; the model is fixed.

Predict whether a dwelling's energy label is E, F or G, from what the
diagnostician recorded. The scorer never changes its model: it fits
scikit-learn's default ``LogisticRegression()`` on the rows you submit, so every
point of AUC comes from your features.
...
* **Any subset of train ids** may appear, at least MIN_TRAIN_ROWS of them: which
  rows to learn from is part of the preprocessing.
