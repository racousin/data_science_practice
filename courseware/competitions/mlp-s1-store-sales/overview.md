...
own files, the unit cost comes from an API and the customer ratings are on a
web page. The work of this challenge is to collect the three sources, join
them, and only then fit a model. To go further, the course database describes
...
**The starter notebook.** One section per source, a baseline model, the cell
that writes and submits `submission.csv`, and a last section on the database.
The code that reads each source is yours to write.
...
### 4. Going further — the database

Once the three sources above are submitted, the course PostgreSQL database
adds a fourth, read with SQL. Its schema `retail` has two tables for this
challenge:
...
The connection string is in the *Access — today's sandbox* section of the
Lab 1.1 page. Store it as a Colab secret or an environment variable, never in
the notebook:
