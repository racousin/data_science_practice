"""Session 3 — Credit risk (file_v1).

Predict whether a loan applicant is a **bad credit risk**, from 20 attributes of
the application. 700 training rows, 30% positive.

The guided half of Session 3. On this training set a random forest reaches
exactly 1.000 accuracy — it memorises all 700 applicants — and still scores
below logistic regression on these held-out rows. A perfect training score is
not a good model; it is a model that has learned the answer key.
...
`prediction` is 1 for a bad credit risk and 0 otherwise. Every test id must
appear exactly once; extra ids are an error, and so is any value other than 0
or 1 — a probability is rejected rather than silently thresholded, because the
threshold is the submitter's decision to make.

Primary score is F1 on the positive (bad-risk) class. Always predicting 0
scores 70% accuracy and an F1 of 0.00, which is why accuracy does not rank.
                ...
                for line_no, row in enumerate(reader, start=2):
                    ...
                    if value not in VALID:
                        return None, (
                            ...
                            f"expected 0 or 1 (1 means bad credit risk)."
                        )
