...
### 1l. Session 3 gains *Stacking and Blending*, 2026-09-20

A tenth teaching lesson in `s3-tabular-models`, between *Time Series Models*
and Lab 3, slug `stacking-and-blending`, 50 authored minutes. **Live**: lesson
**#210** of module #21, published 2026-09-21 by `make publish COURSE=…
MODULE=s3-tabular-models`, which the guard cleared with "in sync with the last
publish — nothing on the website to lose". `check-sync` then reported **0
differences over structure + 11 bodies**, and a walk with the student key
confirmed the learner view is byte-identical to the author view, the title
matches the H1, the lesson sits between `time-series-models` and `lab-3`, and
all **9 images return 200** with no repo-relative path left in the body.

It takes over the two slides *Trees and Ensembles* spent on the subject, which
are now one forward-pointing slide there ("Beyond bagging and boosting"); the
one-to-three-percent expectation stays in both places. `stacking.jpg` moved
with the material, so no image is used twice.

Three stills come from the owner's website,
`data-science-practice/module6/course/ensemble-techniques` — `votbag.png`,
`stacking.jpg` (already in the tree) and `blending.jpg` (copied in from
`website/public/assets/data-science-practice/module6/`). The other six figures
are new and generated by `tools/figures/ms2a_s3_stacking.py`, which runs the
experiment the lesson quotes rather than sketching it: six base models (spline
ridge, 10-NN, MLP, random forest, extra trees, histogram boosting) over six
train/test splits of 6,000 rows, on **two** datasets, and asserts every number
the markdown states.

| test RMSE | California housing | Complementary (generated) |
|---|---|---|
| best single | 0.504 | 1.510 |
| equal average of six | 0.544 (−7.9%) | 2.208 (−46%) |
| blend, one 80/20 split | 0.536 (−6.4%) | 1.530 (−1.3%) |
| stack, 5-fold OOF | 0.502 (**+0.4%**) | 1.458 (**+3.5%**) |

That left column is the lesson's point and it is why the ladder is measured on
two datasets: where one booster dominates (weight 0.89 of 1.0), stacking buys
nothing, and a student should ship the single model. The right column is the
case stacking exists for.

Two other measured claims, both asserted in the generator:

- **The two-model frontier.** The equal-weight average beats the better of two
  models exactly when $\sigma_2/\sigma_1 < \sqrt{\rho^2+3} - \rho$. Over all 180
  model pairs of the experiment the frontier misclassifies **0**, so
  `stack-diversity.png` is a check of an identity, not a scatter with a trend
  drawn through it.
- **The leak.** A 1-NN base model (in-sample RMSE exactly 0) takes weight 1.00
  from a meta-model fitted on in-sample predictions and 0.01 from one fitted
  out-of-fold; test RMSE 0.848 against 0.502, 69% worse.

Gates before the publish: deck builds (345 slides, was 322), **0 overflowing,
0 over-wide code lines, 0 unrenderable inline math** for `s3-tabular-models`;
67 tool tests pass. Four slides were split to clear the overflow check, and `\tfrac`,
`\text{}` and `\frac12` were replaced — matplotlib mathtext parses none of
them, and the first two only fail at build time.

**Not done, deliberately:** Lab 3 does not mention stacking. It is already 90
minutes on a CRPS challenge where the ladder above says the gain would be
inside the weather noise, so the lesson points at the lab and not the other way
round.
