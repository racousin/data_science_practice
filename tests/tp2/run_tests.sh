...
USERNAME=$1
PACKAGE_DIR="${USERNAME}/tp2/mysupertools"
TESTS_DIR="../../../tests/tp2/mysupertools/tests"  # Adjust this path if necessary
...
if [ "$?" -eq 0 ]; then
  echo "All tests passed successfully for tp2."
  deactivate
else
  echo "Tests failed for tp2."
  deactivate
  exit 1
fi
