...
const RNNBackpropagation = () => {
  return (
    <Stack spacing="xl" className="w-full">
      ...
      {/* The Full RNN Backpropagation Algorithm */}
      <Stack spacing="md">
        <Title order={2} id="rnn-backprop-algorithm">The Complete RNN Backpropagation Algorithm</Title>
        ...
        <List>
          <List.Item>Compute the hidden state using the appropriate RNN, LSTM, or GRU equations</List.Item>
          <List.Item>Compute the output: <InlineMath>{`y^{(t)} = f(W_{hy}h^{(t)} + b_y)`}</InlineMath></List.Item>
          <List.Item>Compute the loss at time step t: <InlineMath>{`L^{(t)}`}</InlineMath></List.Item>
        </List>
        <Text>
          Compute the total loss: <InlineMath>{`L = \\frac{1}{T}\\sum_{t=1}^{T}L^{(t)}`}</InlineMath>
        </Text>
        ...
        <Title order={4} mt="sm">For Standard RNN:</Title>
        <List>
          <List.Item>Compute <InlineMath>{`\\frac{\\partial L}{\\partial y^{(t)}}`}</InlineMath> (depends on the loss function)</List.Item>
          <List.Item>Compute <InlineMath>{`\\delta^{(t)} = \\frac{\\partial L}{\\partial h^{(t)}} = \\frac{\\partial L}{\\partial y^{(t)}}W_{hy}^T + \\delta^{(t+1)}W_{hh}^T\\text{diag}(1-(h^{(t+1)})^2)`}</InlineMath></List.Item>
          <List.Item>Accumulate gradients:
            <List withPadding>
              <List.Item><InlineMath>{`\\frac{\\partial L}{\\partial W_{hy}} += \\frac{\\partial L}{\\partial y^{(t)}}(h^{(t)})^T`}</InlineMath></List.Item>
              <List.Item><InlineMath>{`\\frac{\\partial L}{\\partial b_y} += \\frac{\\partial L}{\\partial y^{(t)}}`}</InlineMath></List.Item>
              <List.Item><InlineMath>{`\\frac{\\partial L}{\\partial W_{xh}} += \\delta^{(t)}\\text{diag}(1-(h^{(t)})^2)(x^{(t)})^T`}</InlineMath></List.Item>
              <List.Item><InlineMath>{`\\frac{\\partial L}{\\partial W_{hh}} += \\delta^{(t)}\\text{diag}(1-(h^{(t)})^2)(h^{(t-1)})^T`}</InlineMath></List.Item>
              <List.Item><InlineMath>{`\\frac{\\partial L}{\\partial b_h} += \\delta^{(t)}\\text{diag}(1-(h^{(t)})^2)`}</InlineMath></List.Item>
            </List>
          </List.Item>
        </List>
        
        <Title order={4} mt="sm">For LSTM:</Title>
        <List>
          <List.Item>Compute <InlineMath>{`\\frac{\\partial L}{\\partial y^{(t)}}`}</InlineMath></List.Item>
          <List.Item>Compute <InlineMath>{`\\delta_h^{(t)} = \\frac{\\partial L}{\\partial h^{(t)}}`}</InlineMath></List.Item>
          <List.Item>Compute <InlineMath>{`\\delta_c^{(t)} = \\frac{\\partial L}{\\partial c^{(t)}}`}</InlineMath></List.Item>
          <List.Item>Compute gradients for gates (forget, input, output) and cell candidate</List.Item>
          <List.Item>Accumulate gradients for all weight matrices and bias vectors</List.Item>
        </List>
        
        <Title order={4} mt="sm">For GRU:</Title>
        <List>
          <List.Item>Compute <InlineMath>{`\\frac{\\partial L}{\\partial y^{(t)}}`}</InlineMath></List.Item>
          <List.Item>Compute <InlineMath>{`\\delta_h^{(t)} = \\frac{\\partial L}{\\partial h^{(t)}}`}</InlineMath></List.Item>
          <List.Item>Compute gradients for update gate, reset gate, and candidate hidden state</List.Item>
          <List.Item>Accumulate gradients for all weight matrices and bias vectors</List.Item>
        </List>
        ...
        <Text>
          (Similarly for all weights in LSTM and GRU)
        </Text>
      </Stack>

      {/* Optimization Techniques */}
      <Stack spacing="md">
        <Title order={2} id="optimization-techniques">Optimization Techniques for RNN Training</Title>
        
        <Title order={3}>Truncated BPTT</Title>
        <Text>
          For very long sequences, full BPTT can be computationally expensive. Truncated BPTT limits the number of time steps for backpropagation:
        </Text>
        
        <BlockMath>{`
          \\delta^{(t)} = 
          \\begin{cases} 
          \\frac{\\partial L}{\\partial y^{(t)}}W_{hy}^T + \\delta^{(t+1)}\\frac{\\partial h^{(t+1)}}{\\partial h^{(t)}} & \\text{if } t > t_{min} \\\\
          \\frac{\\partial L}{\\partial y^{(t)}}W_{hy}^T & \\text{if } t = t_{min}
          \\end{cases}
        `}</BlockMath>
        
        <Text>
          Where <InlineMath>{`t_{min} = \\max(1, t-k)`}</InlineMath> and k is the truncation length.
        </Text>
        
        <Title order={3} mt="lg">Gradient Clipping</Title>
        <Text>
          To address exploding gradients, gradient clipping scales down gradients when their norm exceeds a threshold:
        </Text>
        ...
        <Title order={3} mt="lg">Orthogonal Initialization</Title>
        <Text>
          Initializing recurrent weight matrices with orthogonal matrices helps control gradient flow:
        </Text>
        
        <BlockMath>{`
          W_{hh} = \\text{orthogonal matrix with eigenvalues close to 1}
        `}</BlockMath>
        
        <Text>
          This approach helps maintain gradient magnitudes during backpropagation.
        </Text>
      </Stack>
...
export default RNNBackpropagation;