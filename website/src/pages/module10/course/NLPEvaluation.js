...
def llm_translation_evaluation(source_text, translation, reference, source_language, target_language):
    ...
    return {
      ...
      <section id="practical-eval" className="mb-5">
        <h2>5. Practical Evaluation Framework</h2>
        
        <p>
          A comprehensive approach to evaluating LLMs for specific use cases.
        </p>
        ...
        <Row className="mb-4">
          <Col>
            <Card>
              <Card.Body>
                <h5>Multi-dimensional Evaluation Workflow</h5>
                <CodeBlock language="python" code={`import pandas as pd
import numpy as np
from sklearn.metrics import accuracy_score, precision_recall_fscore_support
from llm_eval_utils import call_llm, evaluate_with_llm_judge

def comprehensive_llm_evaluation(model_name, test_dataset, reference_outputs=None):
    """
    ...
    # 1. Task Performance (accuracy-based)
    predictions = []
    for example in test_dataset:
        response = call_llm(model_name, example["input"])
        predictions.append(response)
    
    # Calculate task-specific metrics
    if test_dataset[0]["task_type"] == "classification":
        accuracy = accuracy_score([ex["correct_label"] for ex in test_dataset], 
                                  [extract_label(pred) for pred in predictions])
        precision, recall, f1, _ = precision_recall_fscore_support(
            [ex["correct_label"] for ex in test_dataset],
            [extract_label(pred) for pred in predictions],
            average="weighted"
        )
    ...
    for i, example in enumerate(test_dataset):
        reasoning_score = evaluate_with_llm_judge(
            example["input"],
            predictions[i],
            reference_outputs[i] if reference_outputs else None,
            criteria="reasoning,logical_flow,evidence"
        )
        reasoning_scores.append(reasoning_score)
    
    results["reasoning_quality"] = {
        "average_score": np.mean([s["overall"] for s in reasoning_scores]),
        "logical_coherence": np.mean([s["logical_flow"] for s in reasoning_scores]),
        "evidence_use": np.mean([s["evidence"] for s in reasoning_scores])
    }
    
    # 3. Output Quality (style, tone, helpfulness)
    quality_results = evaluate_outputs_quality(predictions, test_dataset)
    results["output_quality"] = quality_results
    
    # 4. Safety Evaluation
    safety_results = evaluate_safety(model_name, test_dataset)
    results["safety"] = safety_results
    
    return results`} />
                
                <h5>Visualization of Evaluation Results</h5>
                <p>
                  Effective visualization helps identify strengths and weaknesses across dimensions.
                </p>
                <CodeBlock language="python" code={`import matplotlib.pyplot as plt
import seaborn as sns

def plot_model_comparison(evaluation_results, model_names):
    """Create radar chart comparing models across key dimensions"""
    # Extract key metrics
    metrics = ["Task Accuracy", "Reasoning Quality", "Helpfulness", 
               "Factuality", "Instruction Following", "Safety"]
    
    # Prepare data for plotting
    data = []
    for model in model_names:
        model_scores = []
        model_scores.append(evaluation_results[model]["task_performance"]["accuracy"])
        model_scores.append(evaluation_results[model]["reasoning_quality"]["average_score"])
        model_scores.append(evaluation_results[model]["output_quality"]["helpfulness"])
        model_scores.append(evaluation_results[model]["output_quality"]["factuality"])
        model_scores.append(evaluation_results[model]["output_quality"]["instruction_following"])
        model_scores.append(evaluation_results[model]["safety"]["overall_safety_score"])
    ...
    plt.title("Model Comparison Across Evaluation Dimensions")
